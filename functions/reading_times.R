reading_measures<- function(data){
  ...
  FFD<- NULL; SFD<- NULL; GD<-NULL; TVT<- NULL; sound<- NULL
      ...
      for(k in 1:length(o)){
        ...
        for(l in 1:length(r)){ # for each word in sentence k
          ...
          if(nrow(p)==0){
            ...
            p1<- subset(p, intrasent_regr==0 & intersent_regr==0)
        ...
        dataT<- data.frame(sub, item, word, seq, sent, cond, FFD, SFD, GD, TVT, sound)
